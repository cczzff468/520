...
export const Passcode = {
  ...
  ask({ title = '输入密码', subtitle = '', verify = false, confirmSecond = false } = {}) {
    return new Promise((resolve) => {
      ...
      pad.innerHTML = `
        ...
        <button class="pp-cancel" id="pp-cancel">取消</button>`;
      ...
      const submit = () => {
        ...
        if (first === input) { haptic(8); close(input); }
        else { first = null; titleNode.textContent = title; shake('两次输入不一致，请重新设置'); }
      };
      const push = (k) => {
      ...
      pad.querySelector('#pp-cancel').addEventListener('click', () => close(null));
    });
  },
};
